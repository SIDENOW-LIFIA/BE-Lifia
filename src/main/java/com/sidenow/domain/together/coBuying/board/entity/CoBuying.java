package com.sidenow.domain.together.coBuying.board.entity;
...
@AllArgsConstructor
@Entity
public class CoBuying extends BaseTimeEntity {

    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    @Column(name = "coBuying_id")
    private Long coBuyingId; // 게시글 고유 ID
    ...
    private List<CoBuyingComment> coBuyingComments;

    // 게시글 삭제 시 달려있는 좋아요 모두 삭제
    @JsonIgnore // 무한 순환 참조 방지
    @OneToMany(mappedBy = "coBuying", cascade = CascadeType.ALL, orphanRemoval = true)
    private List<CoBuyingLike> coBuyingLikes;
