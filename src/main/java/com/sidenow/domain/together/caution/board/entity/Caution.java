package com.sidenow.domain.together.caution.board.entity;
...
public class Caution extends BaseTimeEntity {
    ...
    private Long cautionId; // 게시글 고유 ID
    ...
    @OneToMany(mappedBy = "caution", cascade = CascadeType.ALL, orphanRemoval = true)
    private List<CautionComment> cautionComments;

    // 게시글 삭제 시 달려있는 좋아요 모두 삭제
    @JsonIgnore // 무한 순환 참조 방지
    @OneToMany(mappedBy = "caution", cascade = CascadeType.ALL, orphanRemoval = true)
    private List<CautionLike> cautionLikes;
