...
@AllArgsConstructor
@Entity
public class Childcare extends BaseTimeEntity {

    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    @Column(name = "childcare_id")
    private Long childcareId; // 게시글 고유 ID
    ...
    @OneToMany(mappedBy = "childcare", cascade = CascadeType.ALL, orphanRemoval = true)
    private List<ChildcareComment> childcareComments;

    // 게시글 삭제 시 달려있는 좋아요 모두 삭제
    @JsonIgnore // 무한 순환 참조 방지
    @OneToMany(mappedBy = "childcare", cascade = CascadeType.ALL, orphanRemoval = true)
    private List<ChildcareLike> childcareLikes;
